function displayCanvas(){
    ...
    //Радиус расположения рисочек
    var radiusPoint;
    contextHTML.lineWidth=3;
    for(var tm = 0; tm < 60; tm++){
        contextHTML.beginPath();
        if(tm % 5 == 0){radiusNum = radiusClock - 30;}else{radiusNum = radiusClock - 15;} //для выделения часовых рисочек
        var xPointM = xCenterClock + radiusClock * Math.cos(-6*tm*(Math.PI/180) + Math.PI/2);
        var yPointM = yCenterClock - radiusClock * Math.sin(-6*tm*(Math.PI/180) + Math.PI/2);
        var xPointM1 = xCenterClock + radiusNum * Math.cos(-6*tm*(Math.PI/180) + Math.PI/2);
        var yPointM1 = yCenterClock - radiusNum * Math.sin(-6*tm*(Math.PI/180) + Math.PI/2);
        contextHTML.moveTo(xPointM, yPointM);
        contextHTML.lineTo(xPointM1,yPointM1);
    ...
    var t_sec = 6*d.getSeconds()+ms/1000*6;                 //Определяем угол для секунд
...
window.onload = displayClock();
    ...
    // var pc = document.getElementById("image");
    // pc.innerHTML="<canvas height='480' width='480' id='myCanvas'></canvas>";
    window.setInterval(
        function(){
            displayCanvas();
        }
